...
const ShaderBackground: React.FC = () => {
  ...
  const fsSource = `
    ...
    const float lineAmplitude = 0.4;
    const float lineFrequency = 0.15;
    const float warpSpeed = 0.12 * overallSpeed;
    const float warpFrequency = 0.35;
    ...
    void main() {
      ...
      vec4 lines = vec4(0.0);
      vec4 bgColor1 = vec4(0.02, 0.06, 0.04, 1.0);
      vec4 bgColor2 = vec4(0.04, 0.12, 0.10, 1.0);
      ...
      for(int l = 0; l < linesPerGroup; l++) {
        ...
        // Custom green-to-teal color mapping
        vec4 activeLineColor = vec4(0.15, 0.85, 0.35, 1.0); // Vibrant green
        float modVal = mod(float(l), 3.0);
        if (modVal < 0.1) {
          activeLineColor = vec4(0.05, 0.85, 0.75, 1.0); // Luminous teal strand
        } else if (modVal < 1.1) {
          activeLineColor = vec4(0.0, 0.95, 0.55, 1.0); // Bright emerald strand
        }

        lines += line * activeLineColor * rand;
      }
